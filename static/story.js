...
// PDF olarak hikayeyi indirme işlevi
function downloadPDF() {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();

    doc.setFont("helvetica", "normal"); // Türkçe karakter desteği için uygun bir font seçiyoruz
    doc.setFontSize(12); // Font boyutunu ayarlıyoruz

    const storyContent = document.getElementById("storyText").innerText; // Hikaye içeriğini al
    doc.text(storyContent, 10, 10, {
        maxWidth: 180, // Satır uzunluğunu sınırlamak için maxWidth kullanıyoruz
    });
